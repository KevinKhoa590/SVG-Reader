    ...
    parseNode = [&](xml_node<>* node, SVGGroup* parentGroup) {
        ...
        if (tag == "line") {
            ...
            for (xml_attribute<>* attr = node->first_attribute(); attr; attr = attr->next_attribute()) {
                std::wstring name(attr->name(), attr->name() + strlen(attr->name()));
                if (name == L"fill" || name == L"stroke") {
                    float op = (name == L"fill")
                        ? parseOpacity(node->first_attribute("fill-opacity"))
                        : parseOpacity(node->first_attribute("stroke-opacity"));

                    const Gdiplus::Color* color = makeColor(attr, op);
                    result.colorPool.push_back(std::make_unique<Gdiplus::Color>(*color));
                }
                path->setAttribute(attr);
            }

            addTo(std::unique_ptr<SVGElement>(path));
        }
        else if (tag == "g") {
