...
// but only if the attributes are present on the node (we don't overwrite with defaults here).
...
// narrow wstring (ASCII assumed for attr strings)
...
// Parse opacity from a wstring (like from parentGroup->getFillOpacity()); returns default if parse fails.
...
// Behavior:
//  - raw empty => returns nullptr (meaning "unspecified")
//  - raw == "none" => returns a fully transparent Color (alpha based on opacity parameter)
//  - otherwise parse color; on unknown format, returns nullptr
Gdiplus::Color* makeColorFromString(SVGParseResult& result, const std::string& rawIn, float opacity) {
    ...
    // return transparent Color for "none" (explicit request)
    if (low == "none") {
    ...
    // named color table (expand as needed)
    ...
    if (!parsed) {
        // unknown -> return nullptr (treat as unspecified)
        return nullptr;
    }
...
float effectiveOpacity(xml_node<>* node, const char* attrName, SVGGroup* parentGroup, float def) {
    if (node) {
        if (auto a = node->first_attribute(attrName)) {
            return parseOpacity(a); // helper from helper.h (accepts xml_attribute<>*)
        }
    }
    if (parentGroup) {
...
// small helper: convert SimpleColor to Gdiplus::Color
...
// if units are OBJECT_BOUNDING_BOX we interpret plain numbers as fractions (0..1)
// if units are USER_SPACE_ON_USE we interpret plain numbers as user coords.
    ...
    if (t.back() == '%') {
        try {
            ...
            if (lg && lg->units == SVGGradient::Units::OBJECT_BOUNDING_BOX) {
                ...
                // user space percent — interpret relative to bounds too
                return isX ? bounds.X + bounds.Width * static_cast<float>(frac)
                    : bounds.Y + bounds.Height * static_cast<float>(frac);
            }
        }
...
// Helper: parse a coordinate for radial gradients (handles percentages vs user space)
// - If units == OBJECT_BOUNDING_BOX: plain numbers are treated as fractions (0..1) mapping to bounds
// - If units == USER_SPACE_ON_USE: plain numbers are absolute user coordinates
...
double parseOffsetString(const std::string& s) {
    ...
    try {
        ...
        // If value looks like > 1 (some tools write "100" meaning percent) treat >1 as percent
        double v = std::stod(t);
...
// resolve a paint string into either a solid color (returned) or a gradient reference (outGrad set to raw pointer).
// If paintStr is "url(#id)" and the id is found, outGrad is set and the function returns a fallback solid color derived from average of gradient (alpha already applied).
// Otherwise returns result of makeColorFromString(...) (which may return nullptr for "none").
