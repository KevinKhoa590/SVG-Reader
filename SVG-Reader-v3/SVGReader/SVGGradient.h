...
struct GradientStop {
    ...
    SimpleColor color;
    double opacity; // 0.0 .. 1.0 (multiplies color.a)
};
...
public:
    ...
    Units units;
    std::string id;
    // optional transform string for gradientTransform (store raw; parser may parse matrix later)
    std::string gradientTransform;

    // optional href: if gradient references another gradient via xlink:href
    std::string href;
