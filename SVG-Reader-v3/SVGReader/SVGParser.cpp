...
// Parse <defs> children (linearGradient/radialGradient) and populate result.gradients + result.gradientsById
void SVGParser::parseDefs(xml_node<>* defsNode, SVGParseResult& result) {
    ...
    // Resolve href copies (we temporarily stored "href:ID" in gradientTransform)
    for (auto& u : result.gradients) {
        ...
        if (!g->href.empty()) {
            ...
            if (it != result.gradientsById.end()) {
                ...
                // clear marker (keep gradientTransform cleared so real transforms can be set by attributes if present)
            }
