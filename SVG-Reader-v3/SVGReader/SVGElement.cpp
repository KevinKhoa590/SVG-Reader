...
std::unique_ptr<Brush> SVGElement::createFillBrush(Graphics& g, const RectF& bounds) const {
    ...
    if (auto lg = dynamic_cast<SVGLinearGradient*>(grad)) {
        // Default coords: objectBoundingBox (0,0) -> (1,0) mapped to bounds
        ...
        if (!stops.empty()) {
            ...
            for (const auto& s : stops) {
                double off = s.offset;
                // treat offsets >1 as percentages (0..100)
            ...
            // Set interpolation using raw arrays (safe: arrays exist here while call is made)
            brush->SetInterpolationColors(colors.data(), positions.data(), n);
            brush->SetWrapMode(WrapModeClamp);
        }

        // gradientTransform (if present) - apply to brush
        if (!lg->gradientTransform.empty()) {
            // convert to wstring and parse using helper (parseTransform returns Matrix*)
            std::wstring w = std::wstring(lg->gradientTransform.begin(), lg->gradientTransform.end());
            Gdiplus::Matrix* m = parseTransform(w);
            if (m) {
                // Multiply the brush transform by the parsed matrix
                brush->MultiplyTransform(m); // MultiplyTransform accepts Matrix*
                delete m;
            }
        }
    ...
    if (auto rg = dynamic_cast<SVGRadialGradient*>(grad)) {
        // Compute center (cx, cy), radius r, focal (fx, fy).
        // Defaults: center at object bbox center, radius = half max dimension, focal = center
        ...
        // Parse attributes (respect gradientUnits stored in rg->units)
        ...
        if (!rg->r.empty()) {
            // If units == OBJECT_BOUNDING_BOX numeric r is fraction of bbox (we interpret as fraction of max dimension)
            ...
            if (usedUserSpace) {
                // treat as absolute user-space radius
                ...
                if (isPercent) {
                    ...
                    // plain number interpreted as fraction
        ...
        // Create circular path for radial gradient
        GraphicsPath path;
        ...
        if (!stops.empty()) {
            ...
            // Center color = color at offset 0 (approx)
            Color centerCol = items.front().second;
            brush->SetCenterColor(centerCol);

            // Surround color(s) = color at offset 1 (approx)
    ...
    // Fallback: average color to a solid brush
